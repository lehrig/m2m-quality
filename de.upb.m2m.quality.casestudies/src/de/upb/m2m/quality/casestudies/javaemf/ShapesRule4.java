...
import de.upb.m2m.quality.casestudies.Shapes.impl.SquareImpl;
 ...
 */
public class ShapesRule4 {
	/**
	 * The Apache logger.
	 */
	private static final Logger LOGGER = Logger.getLogger(ShapesRule4.class);
	 ...
	 */
	public ShapesRule4(String sourceShapesPath, String targetShapesPath) {
	...
	public void transform() {
		...
		List<SquareImpl> squares = allSubobjectsOfKind(targetRootBlock,
				SquareImpl.class);

		for (Square square : squares) {
			createBlock(square);
			EcoreUtil.delete(square);
		}
	...
	/**
	 * Creates a new block by using information from a given square. It also
	 * nests a generated circle into its model elements.
	 * 
	 * @param square
	 *            The square to be used as a template
	 */
	private void createBlock(Square square) {
		Block block = factory.createBlock();
		block.setName(square.getName());
		block.setBlock(square.getBlock());
		block.getInArrow().addAll(square.getInArrow());
		block.getOutArrow().addAll(square.getOutArrow());
		block.getModelElement().add(createCircle(block));
	}

	/**
	 * Creates a new circle which is nested into the given block.
	 * 
	 * @param block
	 *            The block to which the circle is nested to
	 * @return The newly created circle
	 */
	private Circle createCircle(Block block) {
		Circle circle = factory.createCircle();
		circle.setName("generated circle");
		circle.setBlock(block);
		return circle;
	}
	...
	public static void main(String[] modelPaths) {
		ShapesRule4 shapesRule4;
		try {
			shapesRule4 = new ShapesRule4(modelPaths[0], modelPaths[1]);
			shapesRule4.transform();
			shapesRule4.save();
		} catch (Exception e) {
